...
{
  return (tm_zone == TM_REF_UTC || tm_zone == tm_utctimezone ()
          || (tm_zone && tm_utctimezone () && !strcmp (tm_zone, tm_utctimezone ())))? 1 : 0;
}
...
tm_status
tm_make_dtrc (struct tm *tm, int year, tm_month month, int day, int hour, int min, int sec,
              const char *rep, tm_time_precedence clock)
{
  if (rep == TM_REF_UNCHANGED)
    rep = tm_getwallclock (*tm);
  rep = tm_getregisteredwallclock (rep, 1);
...
tm_setfromiso8601 (struct tm *dt, char *str)
{
  ...
  {
    wc = TM_REF_UTC;
    if (strncmp ("Z", str + pmatch[UTCOFFSET].rm_so, pmatch[UTCOFFSET].rm_eo - pmatch[UTCOFFSET].rm_so) == 0)   // UTC time zone
    ...
    if (pmatch[UTCOFFSETSIGN].rm_so >= 0 && pmatch[UTCOFFSETSIGN].rm_so < pmatch[UTCOFFSETSIGN].rm_eo &&        // if UTC offset does not start with '+'
        strncmp ("+", str + pmatch[UTCOFFSETSIGN].rm_so,
                 pmatch[UTCOFFSETSIGN].rm_eo - pmatch[UTCOFFSETSIGN].rm_so) != 0)
      gsign = -1;               // '-' or '−'
    else
      gsign = 1;                // '+'
  }
...
#define MAXLEN ((size_t)6)
    ...
    if (pmatch[parts[part].index].rm_eo - pmatch[parts[part].index].rm_so > MAXLEN)
      substr[parts[part].len = MAXLEN] = 0;
    else
      substr[parts[part].len = pmatch[parts[part].index].rm_eo - pmatch[parts[part].index].rm_so] = 0;
...
tm_timetostring (struct tm dt, size_t max, char *str)
{
  ...
  size_t index = 2 * (tm_isinsidedaylightsavingtimeoverlap (dt) ? 1 : 0) + (tm_isdefinedinlocaltime (dt) ? 0 : 1);
...
{
  struct tm debut, fin;
  if (tm_set (&debut, year, TM_JANUARY, 1, 0, 0, 0) == TM_OK &&
